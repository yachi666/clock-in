...
- 通过免费公开 API 拉取中国大陆年度法定节假日 / 调休日历
- 将年度工作日规则缓存到本机 `SwiftData`
- 向统计模块提供“应到工作日”计算基础
- 标记数据来源、缓存时间和可用性状态，避免统计静默失真

默认接入策略为“免费公开 API + 手机本地缓存”。API 必须支持中国大陆的法定节假日和调休补班，不能只返回普通 public holiday。可选数据源包括 `jiejiariapi.com` 这类免费节假日 API，或提供公开年度 JSON 的同类服务；实现上应通过轻量数据源适配层隔离具体服务，便于后续更换。
...
8. `HolidayService` 通过免费公开 API 拉取年度工作日规则，缓存到本机后供 Dashboard 计算 “XX Days Present out of XX Working Days”。
...
- **HolidayCalendarCache**
  - 年份与地区（默认中国大陆）
  - 日期列表与类型（工作日 / 法定节假日 / 调休补班）
  - 数据源名称与版本
  - 数据源更新时间
  - 本机缓存更新时间
  - 数据可用性状态（fresh / cached / unavailable）
...
节假日数据采用免费公开 API 获取，并在手机本地缓存。应用请求数据时只传递年份和地区，不上传公司位置、考勤记录或用户身份信息。若 API 不可用：

- 有本地缓存时，继续使用缓存并标记为 `cached`
- 无缓存时，Dashboard 不应展示精确“应到工作日”统计
- 不使用“仅按周末估算”的结果冒充法定工作日统计
